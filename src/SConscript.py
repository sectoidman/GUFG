...
runtime = env.SharedLibrary('engine', backend_src + swig_src,
                            SHLIBPREFIX="_", LIBS=libs)
bmpout = env.Program('bmpout', backend_src + bmpout_src, LIBS=libs)
replay = env.Program('replay', backend_src + replay_src, LIBS=libs)
gufg_exec = env.Program('gufg',backend_src + gufg_exec_src, LIBS=libs)
