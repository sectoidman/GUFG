...
void interface::draw()
{
	...
	for(int i = 0; i < 2; i++){
		if(i == 0) 
			drawGlyph(p[i]->pick()->name, 100, 500, 30, 40, scalingFactor, 0);
		else
			drawGlyph(p[i]->pick()->name, 1000, 500, 30, 40, scalingFactor, 2);
	}
...
int interface::drawGlyph(char * string, int x, int space, int y, int height, float scalingFactor, int just)
{
